...
# when the packet path is unchanged. Take the acceptance baseline immediately before the
# HoL sample so CPU allocation changes cannot masquerade as a scheduler regression. Keep
# the initial sample above as both a netem availability check and a visible drift signal.
...
log "fast path alone (adjacent gate) : ${BASELINE} Mbps"
