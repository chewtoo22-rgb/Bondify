...
- diagnostic/support-bundle redaction and listener exposure;
...
- loopback-only diagnostics plus redacted diagnostics/support bundles;
...
The full diagnostics server is intentionally unauthenticated but is enforced as loopback-only. Any future change that makes it remotely reachable must add an explicit authentication and transport-security design rather than weakening the loopback invariant.
