...
Risk: debug/unsigned binaries are mistaken for production releases, artifacts are replaced, publish credentials are over-privileged, or a release tag bypasses reviewed `main` lineage.
...
- tag-triggered publishing fails closed unless the tagged commit is already contained in `main`.
