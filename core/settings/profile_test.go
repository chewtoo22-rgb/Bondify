...
func TestAdmitNormalizesHost(t *testing.T) {
	...
	if got.RelayHost != "relay.example.com" {
		t.Fatalf("host=%q", got.RelayHost)
	}
	if got.Mode != ModeSpeed || !got.AutoReconnect {
		t.Fatalf("unexpected admitted profile: %+v", got)
	}
}

func TestAdmitImplementedModes(t *testing.T) {
	for _, mode := range []Mode{ModeSpeed, ModeRedundant} {
		p := validProfile()
		p.Mode = mode
		if _, err := Admit(p); err != nil {
			t.Fatalf("mode %q: %v", mode, err)
		}
	}
}

func TestAdmitRejectsReservedModes(t *testing.T) {
	for _, mode := range []Mode{ModeStream, ModeCustom} {
		p := validProfile()
		p.Mode = mode
		assertInvalid(t, p)
	}
...
func TestAdmitAcceptsKeepaliveBoundaries(t *testing.T) {
	for _, secs := range []int{MinKeepaliveSec, MaxKeepaliveSec} {
		...
		p.KeepaliveSec = secs
		if _, err := Admit(p); err != nil {
			t.Fatalf("keepalive %d: %v", secs, err)
		}
	}
}
...
func assertInvalid(t *testing.T, p Profile) {
	t.Helper()
	if _, err := Admit(p); !errors.Is(err, ErrInvalidProfile) {
