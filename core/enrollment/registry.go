...
func NewDeviceRegistry(maxDevices int) (*DeviceRegistry, error) {
	if maxDevices <= 0 { return nil, ErrDeviceCapacity }
	return &DeviceRegistry{maxDevices: maxDevices, accounts: make(map[string]map[string]DeviceRecord)}, nil
}
...
func (r *DeviceRegistry) putLocked(record DeviceRecord) error {
	...
	if _, exists := devices[normalized.DeviceID]; !exists && len(devices) >= r.maxDevices { return ErrDeviceCapacity }
...
func (r *DeviceRegistry) Get(accountID, deviceID string) (DeviceRecord, error) {
	...
	if err != nil || !validDeviceID(deviceID) { return DeviceRecord{}, ErrDeviceRecord }
	r.mu.RLock(); defer r.mu.RUnlock()
	record, ok := r.accounts[accountID][deviceID]
	if !ok { return DeviceRecord{}, ErrDeviceNotFound }
	return record, nil
}

func (r *DeviceRegistry) List(accountID string) ([]DeviceRecord, error) {
	accountID, err := normalizeAccountID(accountID)
	if err != nil { return nil, ErrDeviceRecord }
	r.mu.RLock()
	devices := r.accounts[accountID]
	out := make([]DeviceRecord, 0, len(devices))
	for _, record := range devices { out = append(out, record) }
	r.mu.RUnlock()
...
func (r *DeviceRegistry) removeLocked(accountID, deviceID string) error {
	accountID, err := normalizeAccountID(accountID)
	if err != nil || !validDeviceID(deviceID) { return ErrDeviceRecord }
	devices := r.accounts[accountID]
	if _, ok := devices[deviceID]; !ok { return ErrDeviceNotFound }
	delete(devices, deviceID)
	if len(devices) == 0 { delete(r.accounts, accountID) }
	return nil
}

func normalizeDeviceRecord(record DeviceRecord) (DeviceRecord, error) {
	accountID, err := normalizeAccountID(record.AccountID)
	if err != nil || !validDeviceID(record.DeviceID) || !record.Platform.Valid() || record.EnrolledAt.IsZero() { return DeviceRecord{}, ErrDeviceRecord }
	name, err := NormalizeDeviceName(record.Name)
	if err != nil { return DeviceRecord{}, ErrDeviceRecord }
...
func validDeviceID(deviceID string) bool {
	if len(deviceID) != 32 { return false }
