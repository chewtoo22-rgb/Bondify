...
func TestFileDeviceStoreRoundTripAndPermissions(t *testing.T) {
	...
	if err := store.Put(record); err != nil { t.Fatal(err) }
	info, err := os.Stat(path)
	if err != nil { t.Fatal(err) }
	if info.Mode().Perm() != 0o600 { t.Fatalf("mode = %o, want 600", info.Mode().Perm()) }
	reopened, err := OpenFileDeviceStore(path, 2)
	if err != nil { t.Fatal(err) }
	got, err := reopened.List("acct-a")
	if err != nil { t.Fatal(err) }
	if len(got) != 1 || got[0].DeviceID != record.DeviceID { t.Fatalf("round trip = %#v", got) }
}
...
func TestFileDeviceStoreDeterministicOrdering(t *testing.T) {
	...
	if first < 0 || first >= second || second >= third { t.Fatalf("records not deterministically ordered: %s", text) }
}

func TestFileDeviceStoreRejectsCorruptAndSymlinkState(t *testing.T) {
	dir := t.TempDir()
	bad := filepath.Join(dir, "bad.json")
	if err := os.WriteFile(bad, []byte(`{"version":99,"devices":[]}`), 0o600); err != nil { t.Fatal(err) }
	if _, err := OpenFileDeviceStore(bad, 2); err == nil { t.Fatal("expected corrupt/version state rejection") }
	target := filepath.Join(dir, "target.json")
	if err := os.WriteFile(target, []byte(`{"version":1,"devices":[]}`), 0o600); err != nil { t.Fatal(err) }
	link := filepath.Join(dir, "link.json")
	if err := os.Symlink(target, link); err != nil { t.Fatal(err) }
	if _, err := OpenFileDeviceStore(link, 2); err == nil { t.Fatal("expected symlink rejection") }
}
...
func TestFileDeviceStoreRollsBackOnPersistenceFailure(t *testing.T) {
	...
	if err := store.Put(first); err != nil { t.Fatal(err) }
	...
	if err := store.Put(second); err == nil {
		...
		if err := store.Put(third); err == nil { t.Fatal("expected persistence failure") }
	}
	got, err := store.List("acct-a")
	if err != nil { t.Fatal(err) }
	if len(got) != 1 || got[0].DeviceID != first.DeviceID { t.Fatalf("registry was not rolled back: %#v", got) }
}
...
func TestFileDeviceStoreCapacitySurvivesRestart(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := store.Put(fileStoreTestDeviceRecord("acct-a", strings.Repeat("a", 32), "A")); err != nil { t.Fatal(err) }
	reopened, err := OpenFileDeviceStore(path, 1)
	if err != nil { t.Fatal(err) }
	if err := reopened.Put(fileStoreTestDeviceRecord("acct-a", strings.Repeat("b", 32), "B")); err == nil { t.Fatal("expected capacity rejection after restart") }
}
